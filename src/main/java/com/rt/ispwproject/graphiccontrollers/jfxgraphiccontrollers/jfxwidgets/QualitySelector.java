...
public class QualitySelector extends HBox {
    ...
    // Creates the starsContainer and all the star shaped buttons
    public QualitySelector(int startQualityLevel, int maxQualityLevel, boolean editable)
    {
        super();
    ...
    // If editable is true then the quality level can be changed by clicking on the star buttons otherwise the click will not change the level
    public void setEnable(boolean editable)
    {
        ...
        for(int i = 0; i < buttons.size(); ++i)
        {
            int starIndex = i + 1;
            if (editable)
                buttons.get(i).setOnMouseClicked(e -> setQualityLevel(starIndex));
            else
                buttons.get(i).setOnMouseClicked(null);
        }
    }
